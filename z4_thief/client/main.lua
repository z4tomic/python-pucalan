...
function OpenStealMenu(target, target_id) -- abrir un menu de opciones a que elegir
	ESX.UI.Menu.CloseAll()

	ESX.TriggerServerCallback('esx_thief:getOtherPlayerData', function(data) -- registra un logs en el server
		local elements = {} 

		if Config.EnableCash then -- config para permitir robar dinero
		...
		if Config.EnableBlackMoney then -- config para permitir robar dinero negro
			local blackMoney = 0
		...
		if Config.EnableInventory then -- config para permitir robar el inventario del jugaodr
